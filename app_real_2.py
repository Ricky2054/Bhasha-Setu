...
def predict_sign(model, landmarks, classes):
    ...
    prediction = model.predict(np.expand_dims(processed_landmarks, axis=0))[0]
            ...
            while cap.isOpened():
                ...
                if results.multi_hand_landmarks and elapsed_time >= 3:  # 3-second delay between recognitions
                    for hand_landmarks in results.multi_hand_landmarks:
                        ...
                        # Predict sign
                        current_predictions = predict_sign(model, hand_landmarks.landmark, classes)

                        # Display predicted signs
                        for i, (sign, confidence) in enumerate(current_predictions):
                            cv2.putText(frame, f"{i+1}. {sign} ({confidence:.2f})", (10, 50 + i*30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

                        cv2.putText(frame, "Press 1, 2, or 3 to select", (10, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                ...
                if key == ord('q'):
                    ...
                    if choice < len(current_predictions):
                        selected_sign = current_predictions[choice][0]
                        transcription += selected_sign
                        current_predictions = []
                        last_recognition_time = time.time()
